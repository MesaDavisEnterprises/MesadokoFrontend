...
const LevelList = ({ lesson }) => {
  const [filterLevels, setFilterLevels] = useState([]);

  const { levels } = useLevels();

  useEffect(() => {
    const filterLevels = levels.filter((level) => level.lessonId === lesson.id);
          ...
          <tbody className="">
            {filterLevels.map((level) => (
              <tr key={level.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold text-white rounded-full text-${level.color}-800`}
