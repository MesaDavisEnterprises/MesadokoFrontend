...
const QuizList = ({ lesson }) => {
  const [filterQuizzes, setFilterQuizzes] = useState([]);

  const { quizzes } = useQuizzes();

  useEffect(() => {
    const filterQuizzes = quizzes.filter((quiz) => quiz.lessonId === lesson.id);

    setFilterQuizzes(filterQuizzes);
  }, [quizzes]);
          ...
          <tbody className="">
            {filterQuizzes.map((quiz) => (
              <tr key={quiz.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold text-white rounded-full text-${quiz.color}-800`}
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <Link
                    ...
                    className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded-full mx-2"
                  >
                    View
                  </Link>
